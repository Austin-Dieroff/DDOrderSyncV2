...
namespace DDOrderSync.ViewModels
{
    ...
    class FOBCode
    {
        ...
        public static List<FOBCode> GetValuesTyped()
        {
            List<FOBCode> fOBCodes = new List<FOBCode>();
            ...
            if (Properties.Settings.Default["AllianceDBPath"] != null && Properties.Settings.Default["AllianceDBPath"].ToString() != "")
            {
                try
                {
                    ...
                    using (OleDbConnection connection = new OleDbConnection(connectionString))
                    {
                        ...
                        cmd.CommandText = "SELECT `FOBCode`,`DescText` FROM `FOBCodes`";
                        ...
                        using (OleDbDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                fOBCodes.Add(new FOBCode { Code = reader.GetString(0), Description = reader.GetString(1) });
                            }
                        }
                    }
                }
